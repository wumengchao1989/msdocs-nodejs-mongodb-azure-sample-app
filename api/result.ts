  ...
  implements AfterViewInit, OnChanges, OnDestroy, OnInit
{
  ...
  private get target(): Element | Window {
    ...
    return (
      (typeof el === "string" ? this.document.querySelector(el) : el) || window
    );
  }
  constructor(
    el: ElementRef,
  ...
  private registerListeners(): void {
    ...
    this.removeListeners();
    const el =
      this.target === window ? this.document.body : (this.target as Element);
    this.positionChangeSubscription = this.ngZone.runOutsideAngular(() =>
      merge(
        ...Object.keys(AffixRespondEvents).map((evName) =>
          fromEvent(this.target, evName)
        ),
  ...
  private setPlaceholderStyle(placeholderStyle?: NgStyleInterface): void {
    ...
    this.renderer.setStyle(
      ...
      "cssText",
      getStyleAsText(placeholderStyle)
    );
  ...
  private syncPlaceholderStyle(e: Event): void {
    ...
    this.setAffixStyle(e, { ...this.affixStyle, ...styleObj });
  ...
  updatePosition(e: Event): void {
    ...
    const offsetMode = { top: false, bottom: false }; // Default to `offsetTop=0`.
    if (
      typeof offsetTop !== "number" &&
      typeof this.nzOffsetBottom !== "number"
    ) {
      offsetMode.top = true;
      offsetTop = 0;
    } else {
    ...
    if (scrollTop >= elemOffset.top - (offsetTop as number) && offsetMode.top) {
      ...
      this.setPlaceholderStyle({ width, height: elemSize.height });
    } else if (
      scrollTop <=
        elemOffset.top +
          elemSize.height +
          (this.nzOffsetBottom as number) -
          targetInnerHeight &&
      offsetMode.bottom
    ) {
      ...
      this.setPlaceholderStyle({ width, height: elemOffset.height });
    } else {
      if (
